"""
Service readable simplifié utilisant asyncio.run pour chaque opération DB.
"""
import asyncio
from datetime import datetime
from typing import List, Optional, Dict, Any

from app.core.readable_simple import (
    get_expressions_to_process_simple,
    get_readable_stats_simple,
    update_expression_readable_simple
)
...
class ReadableSimpleService:
    """Service readable simplifié pour Celery."""
    
    def __init__(self):
        self.content_extractor = ContentExtractor()
    
    async def get_readable_stats(self, land_id: int) -> Dict[str, Any]:
        """Récupère les statistiques readable."""
        return await get_readable_stats_simple(land_id)
    
    async def get_expressions_to_process(self, land_id: int, limit: Optional[int] = None, depth: Optional[int] = None):
        """Récupère les expressions à traiter."""
        return await get_expressions_to_process_simple(land_id, limit, depth)
    ...
    async def _process_single_expression(self, expression, merge_strategy: MergeStrategy, enable_llm: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if not extraction_result.success:
                ...
                await update_expression_readable_simple(expression.id, {'readable_at': datetime.utcnow()})
                return {
            ...
            await update_expression_readable_simple(expression.id, updated_data)
