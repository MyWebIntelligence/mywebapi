...
// Tagged content (v1 endpoints - per expression / per land)
export async function getExpressionTaggedContent(expressionId) {
  const response = await client.get('/v1/tags/tagged-content', {
    params: { expression_id: expressionId },
  })
  return response.data
}

export async function getLandTaggedContent(landId) {
  const response = await client.get(`/v1/tags/${landId}/tagged-content`)
  return response.data
}

export async function createTaggedContentV1(data) {
  const response = await client.post('/v1/tags/tagged-content', data)
  return response.data
}

export async function updateTaggedContentV1(id, data) {
  const response = await client.put(`/v1/tags/tagged-content/${id}`, data)
  return response.data
}

export async function deleteTaggedContentV1(id) {
  const response = await client.delete(`/v1/tags/tagged-content/${id}`)
  return response.data
}
